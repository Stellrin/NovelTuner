...
NovelTuner 是一个网络小说处理工具集，提供 EPUB 图片修复、繁简转换、文本换行修复、EPUB 内容转换等多种文本处理功能。项目采用统一的管理系统，通过主脚本访问所有工具。

## 🌟 功能特性

### 📖 EPUB 内容转换工具 (epub_converter)
- **EPUB 解包/打包**：自动解包 EPUB 文件，处理内容后重新打包
- **繁简转换**：支持 EPUB 内的繁体/简体中文互相转换
- **换行修复**：修复 EPUB 内中文文本的异常换行问题
- **保留结构**：只修改文本内容，保留原有的 EPUB 结构和样式
- **批量处理**：支持单个文件或整个目录的批量处理
- **编码兼容**：自动检测多种编码格式的 XHTML 文件
...
# 获取工具帮助
python novel_tuner.py epub_converter --help

# EPUB 图像下载
 python novel_tuner.py image_fixer novel.epub

# EPUB 繁体转简体
python novel_tuner.py epub_converter traditional.epub --to-simplified -o simplified.epub
...
# 批量处理目录中的 EPUB 文件
python novel_tuner.py epub_converter epub_dir/ --to-simplified -o output_dir/
...
```bash
# 简繁转换
python novel_tuner.py epub_converter simplified.epub --to-traditional -o traditional.epub

# 自定义标点符号进行换行修复
python novel_tuner.py epub_converter novel.epub --fix-breaks --punctuation "。！？：；" -o output.epub

# 保留解包文件用于调试
python novel_tuner.py epub_converter novel.epub --to-simplified --keep-extracted -o output.epub

# 指定解包目录
python novel_tuner.py epub_converter novel.epub --extract-dir ./temp_extract -o output.epub

# 带备份的处理
python novel_tuner.py epub_converter novel.epub -b --to-simplified -o converted.epub
```
...
# 带备份的批量目录处理
python novel_tuner.py fix_line_breaks+traditional_to_simplified input_dir/ -r -o output_dir/ -b -v

# 处理流程说明：
# 1. 首先执行 fix_line_breaks 修复换行问题
# 2. 将第一步的输出作为输入执行 traditional_to_simplified
# 3. 中间文件自动管理，处理完成后清理
# 4. 最终输出为最后一个工具的处理结果
```

工具按从左到右的顺序依次执行，前一个工具的输出自动作为后一个工具的输入。
...
- **EPUB 处理工具**：EPUB 解包、打包、内容处理

## 🎯 使用场景

### EPUB 内容转换
- 将繁体中文 EPUB 小说转换为简体中文
- 修复 EPUB 中的中文换行问题
- 批量处理整个目录的 EPUB 文件
- 保留原有 EPUB 结构和样式，仅修改文本内容
...
- **EPUB处理**：EPUB解包/打包、XHTML内容处理、元数据提取
...
8. **EPUB 转换**：EPUB 转换工具会修改文件内容，建议先使用 `-b` 参数创建备份


### 📋 功能状态

#### ✅ 已实现功能
- **EPUB 内容转换**：支持 EPUB 文件的繁简转换和换行修复
- **多工具管道调用**：支持 `tool1+tool2+tool3` 语法链式执行多个工具
- **自动工具发现**：自动扫描并加载所有可用工具
- **统一参数标准**：所有工具支持 `-o, -r, -b, -v, -q` 等通用参数
- **智能编码检测**：支持 UTF-8、GBK、Big5 等多种编码
- **批量文件处理**：支持单个文件和目录递归处理
- **备份管理**：带时间戳的备份创建和恢复功能
